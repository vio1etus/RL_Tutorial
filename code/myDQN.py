...
parser.add_argument('--eps', type=float, default=1)  # 暂时没用到
...
class Agent:
    def __init__(self, env):
        ...
        # self.model_optim = self.target_model_optim = tf.optimizers.RMSprop(
        #     lr=args.lr)
    ...
    def replay(self):
        for _ in range(10):
            ...
            target[range(args.batch_size), actions] = rewards + \
                (1 - done) * args.gamma * next_q_value
            ...
            self.model_optim.apply_gradients(
    ...
    def train(self, train_episodes=200):
        if args.train:
            for episode in range(train_episodes):
                ...
                self.epsilon = 1.00 / (episode + 1)  # 衰减的 epsilon-greedy

                print('EP{} Episode Reward={}'.format(episode, total_reward))
                # if episode % 10 == 0:
                #     self.test_episode()
            self.saveModel()
        if args.test:
    ...
    def saveModel(self):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        tl.files.save_weights_to_hdf5(
            os.path.join(path, 'model.hdf5'), self.model)
        tl.files.save_weights_to_hdf5(os.path.join(
            path, 'target_model.hdf5'), self.target_model)
        print('Saved weights.')
    ...
    def loadModel(self):
        ...
        if os.path.exists(path):
            ...
            tl.files.load_hdf5_to_weights_in_order(
                os.path.join(path, 'model.hdf5'), self.model)
            tl.files.load_hdf5_to_weights_in_order(os.path.join(
                path, 'target_model.hdf5'), self.target_model)
            print('Load weights!')
        else:
